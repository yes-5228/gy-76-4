...
def dashboard_summary():
    ...
    valid_attendance = [item for item in data["attendance"] if not item.get("revoked", False)]
    month_attendance = [item for item in valid_attendance if item["checked_at"].startswith(current_month)]
    ...
    return {
        ...
        "recent_attendance": sorted(valid_attendance, key=lambda item: item["checked_at"], reverse=True)[:5],
    }
